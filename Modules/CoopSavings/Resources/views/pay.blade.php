...
@section('logo', asset(config('coopsavings.logo')))

@section('gateway', config('coopsavings.name'))

@section('content')
    <p class="para-6">{{ __('Fill in the required information') }}</p>
    <div class="straight-line"></div>
    @include('gateway::partial.instruction')
    <form action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('coopsavings.alias')])) }}"
        ...
        <button type="submit" class="pay-button sub-btn">{{ __('Pay with COOP Savings') }}</button>
    </form>
@endsection
